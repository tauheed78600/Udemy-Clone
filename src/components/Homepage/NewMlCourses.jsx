...
function NewMlCourses() {
    ...
    const mlCoursesRef = useRef(null);
    ...
    useEffect(() => {
        ...
        if (mlCoursesRef.current) {
            observer.observe(mlCoursesRef.current);
        }

        return () => {
            if (mlCoursesRef.current) {
                observer.unobserve(mlCoursesRef.current);
            }
        };
    }, [isLoading]);
    ...
    return (
        <div className='lg:ml-[100px] ml-4' ref={mlCoursesRef}>
            {isLoading ? (
                <LoadingSkeleton />
            ) : (
                <div>
